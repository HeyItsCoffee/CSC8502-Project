...
void OGLRenderer::SetShaderLight(const Light& l) {
	glUniform1i(glGetUniformLocation(currentShader->GetProgram(), "lightType"), l.GetType());
...
void OGLRenderer::UpdateShaderMatrices()	{
	if (currentShader) {
		Matrix4* mvpBuffer = new Matrix4[5];
		mvpBuffer[0] = modelMatrix.values;
		mvpBuffer[1] = viewMatrix.values;
		mvpBuffer[2] = projMatrix.values;
		mvpBuffer[3] = textureMatrix.values;
		mvpBuffer[4] = shadowMatrix.values;
		int binding = 5;

		if (firstShaderCall) glGenBuffers(1, &someBuffer);

		glBindBuffer(GL_UNIFORM_BUFFER, someBuffer);
		if (firstShaderCall) {
			glBufferData(GL_UNIFORM_BUFFER, 320, mvpBuffer, GL_DYNAMIC_DRAW);
			firstShaderCall = false;
		}
		else glBufferSubData(GL_UNIFORM_BUFFER, 0, 320, mvpBuffer); // One float (4 bytes) times 16 floats per Mat4 (64 total) times 5 mat4s
		glBindBufferBase(GL_UNIFORM_BUFFER, binding, someBuffer);

		int ubo = glGetUniformBlockIndex(currentShader->GetProgram(), "ObjectMatrices");
		if(ubo >= 0)
